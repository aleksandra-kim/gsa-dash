...
def create_lca(project, db_name, method, functional_unit, fu_amount, seed):
    bd.projects.set_current(project)
    db = bd.Database(db_name)
    fu_location = functional_unit.split(",")[-1][1:]
    fu_name = functional_unit[:-len(fu_location)-2]
    ...
    fu = fu[0]
    method = tuple(method.split("'")[1::2])
    lca = bc.LCA({fu: fu_amount}, method, use_distributions=True, seed_override=seed)  # Remove from sequential
    lca.lci()
    lca.lcia()
    return lca
